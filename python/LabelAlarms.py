...
if __name__ == "__main__":
    for project_config in DataUtils.read_projects(PathUtils.join_path("project.json")).values():
        # 读取警告记录
        ...
        LOG.info("TP: {0}, FP: {1}, UNKNOWN: {2}".format(
            count_df.get(Alarm.TP, default=0), count_df.get(Alarm.FP, default=0),
            count_df.get(Alarm.UNKNOWN, default=0)))
        # 保存结果
